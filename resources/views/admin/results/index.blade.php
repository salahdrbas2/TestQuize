                    ...
                    <table class="table table-bordered table-striped table-hover datatable datatable-result" cellspacing="0" width="100%">
                        ...
                        @can('resultView')
                            ...
                            <tr data-entry-id="{{ $result->id }}">
                                ...
                                <td>
                               {{$result->total_points*100/$all}}%               
                                </td>
                                <td>
                                    @if(($result->total_points*100/$all) > 49)
                                    <span class="badge badge-success">Successful</span>
                                    @endif
                                    @if(($result->total_points*100/$all) < 49)
                                    <span class="badge badge-danger">Not Successful</span>
                                    @endif
                                 </td>
                                <td>
                                    @foreach($result->questions as $key => $question)
                                        <span class="badge badge-info">{{ $question->question_text}} : {{$question->pivot->points }}</span>
                                    @endforeach
                                </td>
                                <td>
...
<script>
  ...
  let deleteButtonTrans = 'Delete All'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.results.DestroyAll') }}",
